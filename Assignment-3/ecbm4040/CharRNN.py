...
class CharRNN():
    ...
    def rnn_layer(self):
        ...
        if self.cell_type =="LSTM" :
            for i in range(int(self.num_layers)):
                cell_list.append(tf.contrib.rnn.LSTMCell(self.rnn_size))
                
        else :
            for i in range(int(self.num_layers)):
                cell_list.append(tf.nn.rnn_cell.GRUCell(self.rnn_size))
                
        #adding dropout
        
        
        for i in range(len(cell_list)):
            cell_list[i] = tf.contrib.rnn.DropoutWrapper(cell_list[i],output_keep_prob=self.keep_prob)
            
        print(len(cell_list))    
            
        multiplecell =tf.contrib.rnn.MultiRNNCell(cell_list)   
        self.initial_state =multiplecell.zero_state(self.batch_size, tf.float32)
        
        
            
        outputs , final_state = tf.nn.dynamic_rnn(multiplecell, self.rnn_inputs,initial_state=self.initial_state)
    ...
    def my_optimizer(self):
        ...
        gradients, x = tf.clip_by_global_norm(tf.gradients(self.loss, variables),self.grad_clip)
        opt = tf.train.AdamOptimizer(self.learning_rate)
        self.optimizer = opt.apply_gradients(zip(gradients, variables))
        ...
        with self.session as sess:
            ...
            for x, y in batches:
                ...
                if (counter % save_every_n == 0):
                    if self.cell_type =="LSTM" :
                        self.saver.save(sess, "checkpoints/lstmi{}_l{}.ckpt".format(counter, self.rnn_size))
                    else :
                        self.saver.save(sess, "checkpoints/grui{}_l{}.ckpt".format(counter, self.rnn_size))
    ...
    def sample(self, checkpoint, n_samples, vocab_size, vocab_to_ind, ind_to_vocab, prime='You \n'):
        ...
        with tf.Session() as sess:
            ...
            for i in range(length):
                index = [[vocab_to_ind[samples[i]]]]
                state,predict = sess.run([self.final_state,self.prob_pred],feed_dict = {self.inputs: index, self.initial_state: state,self.keep_prob:1.})
                
       
            top = pick_top_n (predict, len(ind_to_vocab), top_n=5)
            char = ind_to_vocab[top]
            output.append(char)
            x = np.array([[top]])

            for i in range(n_samples):
                state, predict = sess.run([self.final_state,self.prob_pred],feed_dict={self.inputs: x, self.initial_state: state,self.keep_prob:1.})
