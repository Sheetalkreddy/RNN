...
class CharRNN():
    ...
    def rnn_layer(self):
        ...
        if self.cell_type =="LSTM" :
            ...
            for i in range(int(self.num_layers)):
                cell_list.append(tf.contrib.rnn.GRUCell(self.rnn_size))
        ...
        sliced = [tf.squeeze(input_, [1]) for input_ in inputs]
    
            
        outputs , final_state = tf.contrib.rnn.static_rnn(multiplecell, sliced,initial_state=self.initial_state)
    ...
    def sample(self, checkpoint, n_samples, vocab_size, vocab_to_ind, ind_to_vocab, prime='You \n'):
        ...
        with tf.Session() as sess:
            ...
            new_input = pick_top_n(predict, len(ind_to_vocab), top_n=5)     
                
                
            for i in range(n_samples):    
                state, predict = sess.run([self.final_state,self.prob_pred],feed_dict={self.inputs:new_input, self.initial_state: state,self.keep_prob:1.})
